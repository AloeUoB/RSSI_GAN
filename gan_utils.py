...
map_B = {'a': 'hallway_lower', 'b': 'living_room',   'c': 'dining_room_1',
        ...
        'stairs_upper': 6, 'bathroom': 7, 'hallway_upper': 8, 'bedroom_2': 9, 'bedroom_1':10}
...
def plot_line_rssi_gan(rssi_plot, label_plot, transpose=False, house = None, house_map=None, reduce=False,
                       full_scale=False, ymin=None, ymax=None, save=False, save_dir=None, model_name=None, plot_idx=None):
    if transpose:
    ...
    plt.tight_layout()
    if save:
        plt.savefig(save_dir + model_name + label_plot +'_idx'+str(plot_idx)+'.png')
    else:
        plt.show()
    plt.close()
...
def plot_line_rssi_all(rssi_plot, label_plot, transpose=False, house = None, house_map=None, reduce=False,
    ...
    plt.tight_layout()
    if save:
        plt.savefig('GAN/lineplot_all/House_'+house+'/'+label_plot+'/' + model_name +'.png')
    else:
        plt.show()
    plt.close()
...
def get_mean_stat(windowed_data, windowed_label, number_APs, room_num, save=False, plot_name=None):
    ...
    for room_label in range(room_num):
        ...
        if save:
            plot_line_rssi_gan(ap_plot.cpu().detach().numpy(), room_label + 1, transpose=True, house=None, house_map=None,
                               ymin=-0.1, ymax=1.1, save=True, model_name=str(plot_name))
        else:
            plot_line_rssi_gan(ap_plot.cpu().detach().numpy(), room_label + 1, transpose=True, house=None, house_map=None,
                               ymin=-0.1, ymax=1.1, save=False)

def get_stats(ori_data, label, norm = True ): # house_file = 'csv_house_' + house_name + '_fl.csv'
    if norm:
...
def get_fake_rssi(real_data_labels, num_epochs, total_number, NUM_CLASSES, APs, save_directory, model_name, device):
    # generator parameter
    Z_DIM = 11
    CHANNELS_RSSI = 1
    FEATURES_GEN = 32
