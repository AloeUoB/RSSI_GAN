    ...
    for epoch in range(NUM_EPOCHS):
        ...
        with torch.no_grad():
            ...
            plot_line_rssi_gan(x_fake.cpu().detach().numpy(), cur_labels_numpy[0] + 1, house=house_name,
                               house_map=house_map[house_name], ymin=-0.1, ymax=1.1, save=True,
                               save_dir='GAN/train_visual/fake/', model_name=model_name+'-'+str(epoch), transpose=True)
            plot_line_rssi_gan(x_real.cpu().detach().numpy(), cur_labels_numpy[0] + 1, house=house_name,
                               house_map=house_map[house_name], ymin=-0.1, ymax=1.1, save=True,
                               save_dir='GAN/train_visual/real/', model_name=model_name+'-'+str(epoch), transpose=True)
